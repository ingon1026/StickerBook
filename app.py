...
from render.sticker import StickerAsset
...
class AppAction(Enum):
    ...
    SELECT_MOTION_6 = auto()
    SELECT_MOTION_7 = auto()
    SELECT_MOTION_8 = auto()
    SELECT_MOTION_9 = auto()
    SELECT_MOTION_10 = auto()
...
class App:
    ...
    def _handle_key(self, key: int) -> Optional[AppAction]:
        ...
        if masked == ord("6"):
            return AppAction.SELECT_MOTION_6
        if masked == ord("7"):
            return AppAction.SELECT_MOTION_7
        if masked == ord("8"):
            return AppAction.SELECT_MOTION_8
        if masked == ord("9"):
            return AppAction.SELECT_MOTION_9
        if masked == ord("0"):
            return AppAction.SELECT_MOTION_10
    ...
    def _run_ad_pipeline(
        ...
        self._persist_animation_capture(work_dir, out_dir, result)
        return sticker_asset, result

    def _persist_animation_capture(
        self,
        work_dir: Path,
        out_dir: Path,
        result: AnimationResult,
    ) -> None:
        """Copy AD outputs into assets/captures/<ts>_<motion>/ so the dancing
        GIF survives the work_dir cleanup that fires on R/Q."""
        if result.video_path is None or not result.video_path.is_file():
            return
        motion = (
            self._motion_library.active()
            if self._motion_library is not None and self._motion_library.active()
        ...
        ts = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
        dest = Path(CAPTURES_DIR) / f"{ts}_{motion}"
        try:
            dest.mkdir(parents=True, exist_ok=True)
            shutil.copyfile(result.video_path, dest / result.video_path.name)
            for fname in ("char_cfg.yaml", "bounding_box.yaml", "texture.png", "mask.png"):
                src = out_dir / fname
                if src.is_file():
                    shutil.copyfile(src, dest / fname)
            input_png = work_dir / "input.png"
            if input_png.is_file():
                shutil.copyfile(input_png, dest / "input.png")
            print(f"[app] capture persisted: {dest}")
        except OSError as e:
            print(f"[app] capture persist failed ({dest}): {e}")
            ...
            if future.done():
                try:
                    ...
                    if anchor.is_lost():
                        ...
                        self._promote_to_live(sticker_asset, anchor, anim_result)
        ...
        try:
            while True:
                ...
                if action in (
                    ...
                    AppAction.SELECT_MOTION_5, AppAction.SELECT_MOTION_6,
                    AppAction.SELECT_MOTION_7, AppAction.SELECT_MOTION_8,
                    AppAction.SELECT_MOTION_9, AppAction.SELECT_MOTION_10,
                ):
                    if self._motion_library is not None:
                        idx = {
                            ...
                            AppAction.SELECT_MOTION_6: 6,
                            AppAction.SELECT_MOTION_7: 7,
                            AppAction.SELECT_MOTION_8: 8,
                            AppAction.SELECT_MOTION_9: 9,
                            AppAction.SELECT_MOTION_10: 10,
                        }[action]
